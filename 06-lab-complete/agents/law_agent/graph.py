...
def route_to_subagents(state: LawState) -> list[Send]:
    """Routing function: dispatch parallel Send objects based on keywords."""
    question_lower = state["question"].lower()
    sends: list[Send] = []
    
    # Keyword-based routing to skip the LLM check_routing step
    if any(kw in question_lower for kw in ["tax", "taxes", "irs", "thuế"]):
...
async def call_tax(state: LawState) -> dict:
    ...
    from agents.common.registry_client import discover
...
def create_graph():
    ...
    # Conditional parallel dispatch trực tiếp từ analyze_law
